...
def load_training_validation_data(train_view=None, val_view=None, train_dir=None, val_dir=None):
    ...
    for id in human_id:
        for dir in train_dir:
            img_dir = "%s/%s/%s/%s" % ("/home/sudhamsa/HID project/gei_recognition/data/train", id, dir, train_view)
            if not os.path.exists(img_dir):
                logger.warning("%s do not exist" % img_dir)
        for dir in val_dir:
            img_dir = "%s/%s/%s/%s" % ("/home/sudhamsa/HID project/gei_recognition/data/train", id, dir, val_view)
            if not os.path.exists(img_dir):
                logger.warning("%s do not exist" % img_dir)
